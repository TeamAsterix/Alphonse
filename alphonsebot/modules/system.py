...
from alphonsebot import ALIVE_MSG, BOT_VERSION, CHANNEL, HELP, HOSTNAME, USER
from alphonsebot.modules.ecem import ecem
...
# ================= CONSTANT =================
CUSTOM_MSG = ALIVE_MSG or f"`{get_translation('alphonseAlive')}`"
# ============================================


@alphonseify(pattern='^.neofetch$')
...
def bot_version(message):
    if which('git'):
        ...
        edit(
            message,
            get_translation(
                'alphonseShowBotVersion',
                ['**', '`', CHANNEL, BOT_VERSION, result],
            ),
            preview=False,
        )
    else:
        edit(message, f'`{get_translation("alphonseGitNotFound")}`')


@alphonseify(pattern='^.ping$')
def ping(message):
    start = datetime.now()
    edit(message, '**Pong!**')
    finish = datetime.now()
...
@alphonseify(pattern='^.dc$', compat=False)
...
def terminal(message):
    ...
    try:
        from alphonseecem.core.misc import __status_out__
    ...
    try:
        ...
        if evaluation:
            if isinstance(evaluation, str):
                ...
                edit(
                    message,
                    get_translation('alphonseQuery', ['**', '`', args, evaluation]),
                )
        else:
            edit(
                message,
                get_translation(
                    'alphonseQuery', ['**', '`', args, get_translation('alphonseErrorResult')]
                ),
            )
    except Exception as err:
        edit(message, get_translation('alphonseQuery', ['**', '`', args, str(err)]))
