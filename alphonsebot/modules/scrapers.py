...
from alphonsebot import HELP, ALPHONSE_LANG
...
CARBONLANG = 'auto'
TTS_LANG = ALPHONSE_LANG
TRT_LANG = ALPHONSE_LANG


@alphonseify(pattern='^.crblang')
...
def do_gsearch(query, page):
    ...
    page = find_page(page)
    temp = f'/search?q={query}&start={find_page(page)}&hl={ALPHONSE_LANG}'
...
@alphonseify(pattern='^.d(uckduck|d)go')
def ddgo(message):
    query = extract_args(message)
    if len(query) < 1:
        edit(message, f'`{get_translation("wrongCommand")}`')
...
def urbandictionary(message):
    ...
    if int(meanlen) >= 0:
        ...
        edit(
            message,
            get_translation(
                'alphonseQueryUd', ['**', '`', query, mean[0]['def'], mean[0]['example']]
            ),
        )
    else:
...
@alphonseify(pattern='^.wiki')
def wiki(message):
    args = extract_args(message)
    if len(args) < 1:
        edit(message, f'`{get_translation("wrongCommand")}`')
        return
    set_lang(ALPHONSE_LANG)
    try:
        summary(args)
    ...
    edit(message, get_translation('alphonseQuery', ['**', '`', args, result]))

    send_log(get_translation('wikiLog', ['`', args]))


@alphonseify(pattern='^.tts')
