...
from alphonsebot.modules.chat import is_muted
from alphonseecem.core import edit, get_translation, reply, alphonseify, send_log
...
def pmpermit_init():
    try:
        ...
        sql = import_module('alphonseecem.sql.pm_permit_sql')
...
def permitpm(client, message):
    if not PM_AUTO_BAN:
        ...
        if message.chat.id != 777000:
            try:
                from alphonseecem.sql.pm_permit_sql import is_approved
...
def auto_accept(client, message):
    ...
    if message.chat.id not in [self_user.id, 777000]:
        try:
            from alphonseecem.sql.pm_permit_sql import approve, is_approved
        except BaseException:
            return False
...
@alphonseify(outgoing=True, pattern='^.notifoff$')
...
@alphonseify(outgoing=True, pattern='^.notifon$')
def notifon(message):
    try:
...
@alphonseify(outgoing=True, pattern='^.approve$', compat=False)
def approvepm(client, message):
    try:
        from alphonseecem.sql.pm_permit_sql import approve
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@alphonseify(outgoing=True, pattern="^.disapprove$")
def disapprovepm(message):
    try:
        from alphonseecem.sql.pm_permit_sql import dissprove
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
