...
def snips_init():
    try:
        ...
        sql = import_module('alphonseecem.sql.snips_sql')
...
@alphonseify(pattern='^.addsnip')
def save_snip(message):
    try:
        from alphonseecem.sql.snips_sql import add_snip
...
@alphonseify(pattern='^.snips$')
def snip_list(message):
    try:
        from alphonseecem.sql.snips_sql import get_snips
...
@alphonseify(pattern='^.remsnip')
def delete_snip(message):
    try:
        from alphonseecem.sql.snips_sql import remove_snip
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
def get_snip(message):
    try:
        try:
            from alphonseecem.sql.snips_sql import get_snip
        except BaseException:
            edit(message, f'`{get_translation("nonSqlMode")}`')
            return
