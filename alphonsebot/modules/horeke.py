...
from alphonsebot import HELP, HEROKU_APPNAME, HEROKU_KEY
from alphonseecem.core import edit, get_translation, reply_doc, alphonseify, send_log


@alphonseify(pattern='^.(quo|ko)ta$')
def dyno(message):
    if not HEROKU_KEY:
...
@alphonseify(pattern='^.(restart|yb)$')
def _restart(message):
    return restart(message)


@alphonseify(pattern='^.d(restart|yb)$')
...
@alphonseify(pattern='^.(shutdown|kapat)$')
def shutdown(message):
    ...
    heroku_app.scale_formation_process('alphonse', 0)


@alphonseify(pattern='^.logs$')
def dyno_logs(message):
    ...
    filename = 'alphonse_heroku_log.txt'
