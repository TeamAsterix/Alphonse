...
from alphonsebot import HELP, SUPPORT_GROUP
...
@alphonseify(pattern='^.chatid$', private=False)
def chatid(message):
    edit(message, get_translation('chatidResult', ['`', str(message.chat.id)]))


@alphonseify(pattern='^.invitelink$', compat=False, admin=True, private=False)
...
@alphonseify(pattern='^.kickme$', compat=False, private=False)
def kickme(client, message):
    edit(message, f'`{get_translation("kickmeResult")}`')
    client.leave_chat(message.chat.id, 'me')


@alphonseify(pattern='^.support$')
def support(message):
    edit(message, get_translation('supportResult', [SUPPORT_GROUP]), preview=False)


@alphonseify(pattern='^.founder')
def founder(message):
    edit(message, get_translation('founderResult', ['`', '**']), preview=False)


@alphonseify(pattern='^.readme$')
def readme(message):
    edit(
        message,
        '[README.md](https://github.com/TeamAlphonse/Alphonse/blob/master/README.md)',
        preview=False,
    )


@alphonseify(pattern='^.repo$')
def repo(message):
    edit(
        message,
        '[Repo](https://github.com/TeamAlphonse/' 'Alphonse)',
        preview=False,
    )


@alphonseify(pattern='^.repeat')
def repeat(message):
...
@alphonseify(pattern='^.report$', compat=False, private=False)
