...
def notes_init():
    try:
        ...
        sql = import_module('alphonseecem.sql.notes_sql')
...
@alphonseify(pattern='^.notes$')
def notes(message):
    try:
        from alphonseecem.sql.notes_sql import get_notes
...
@alphonseify(pattern=r'^.save')
def save_note(message):
    try:
        from alphonseecem.sql.notes_sql import add_note
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@alphonseify(pattern=r'^.clear')
def clear_note(message):
    try:
        from alphonseecem.sql.notes_sql import rm_note
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
def get_note(message):
    try:
        try:
            from alphonseecem.sql.notes_sql import get_note
