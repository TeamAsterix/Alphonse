...
from alphonsebot import HELP, HEROKU_APPNAME, HEROKU_KEY, REPO_URL
...
def upstream(ups):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head('alphonse', origin.refs.alphonse)
        repo.heads.alphonse.set_tracking_branch(origin.refs.alphonse)
        repo.heads.alphonse.checkout(True)
    ...
    if force_update:
        ...
        edit(ups, f'`{get_translation("updateAlphonseBot")}`')
