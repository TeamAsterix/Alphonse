...
from alphonsebot import HELP, LOG_ID, LOGS
...
def filters_init():
    try:
        ...
        sql = import_module('alphonseecem.sql.filters_sql')
...
def filter_incoming(message):
    ...
    try:
        from alphonseecem.sql.filters_sql import get_filters
...
@alphonseify(pattern='^.addfilter')
def add_filter(message):
    try:
        from alphonseecem.sql.filters_sql import add_filter
    except BaseException:
...
@alphonseify(pattern='^.stop')
def stop_filter(message):
    try:
        from alphonseecem.sql.filters_sql import remove_filter
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@alphonseify(pattern='^.stopall$')
def stop_filter_all(message):
    try:
        from alphonseecem.sql.filters_sql import get_filters, remove_filter
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
    filters = get_filters(message.chat.id)
