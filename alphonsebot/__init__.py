...
import alphonseecem.translator as _tr
...
ALPHONSE_LANG = environ.get('ALPHONSE_LANG', 'en')


def get_translation(transKey, params: list = None):
    ret = _tr.get_translation(ALPHONSE_LANG, transKey)
...
BOT_VERSION = '0.0.1'
SUPPORT_GROUP = 'AlphonseSupport'
CHANNEL = 'AlphonseUpdates'
...
# For neofetch
HOSTNAME = environ.get('HOSTNAME', 'TeamAlphonse')
USER = environ.get('USER', 'alphonseecem')
...
#Bot Session
SESSION = environ.get('SESSION', 'alphonseify')

#Bot repo url for updater
REPO_URL = environ.get(
    'REPO_URL', 'https://github.com/TeamAlphonse/Alphonse'
)
...
def load_brain():
    try:
        ...
        URL = (
            'https://raw.githubusercontent.com/Ryoishin/'
...
class PyroClient(Client):
    ...
    def start(self):
        ...
        LOGS.info(get_translation('alphonseVersion', [BOT_VERSION]))
...
def __import_modules():
    ...
    for module in modules:
        try:
            import_module(f'alphonsebot.modules.{module}')
        except Exception:
            if LOG_VERBOSE:
