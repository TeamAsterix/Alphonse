...
@alphonse_cmd(pattern="(wc)$")
async def _(event):
    if not event.reply_to_msg_id:
        await edit_delete(event, "`Please reply to any media`")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await edit_delete(event, "`Please Reply To Image/Sticker/Video`")
        return
    xx = await edit_or_reply(event, "`download Media.....`")
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}wc`\
        \n  •  **Function : **make wordcloud art from media.\
    "
    }
)
