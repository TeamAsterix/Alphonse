     ...
     if not graph.text[0].isalpha() and graph.text[0] not in ("/", "#", "@", "!"):
         ...
         if graph.reply_to_msg_id:
             ...
             if input_str == "m":
                 ...
                 await xxnx.edit(
                     f"**Downloaded To** `{downloaded_file_name}` **at** `{ms}` **seconds.**"
                 )
             ...
             await edit_delete(
                 xxnx,
                 "**Please Reply To Message, To Get Permanent Telegraph Link.**",
             )
 ...
 CMD_HELP.update(
     {
         ...
         \n\n • **Syntax :** `{cmd}tg` m\
         \n • **Function : **Uploading m(Media) To Telegraph.\
         \n\n • **Syntax :** `{cmd}tg` t\
         \n • **Function : **Uploading t(Text) To Telegraph.\
     "
     }
 )