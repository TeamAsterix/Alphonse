"""Count the Number of Dialogs you have in your Telegram Account
 ...
 @bot.on(alphonse_cmd(outgoing=True, pattern=r"stats$"))
 async def stats(
 ...
 @bot.on(alphonse_cmd(outgoing=True, pattern=r"(ustat|detect|ustats)(?: |$)(.*)"))
 async def _(event):
     if event.fwd_from:
         return
     ...
     if not input_str and not reply_message:
         await edit_delete(
             events,
             "`Reply user message or provide userid/username`",
         )
     if input_str:
         try:
             uid = int(input_str)
             ...
             try:
                 ...
                 await edit_delete(
                     event, "`Provide userid or username to view group history`"
                 )
             uid = u.id
     else:
     ...
     async with event.client.conversation(chat) as conv:
         try:
             ...
             await edit_delete(sevent, "**Unblock @tgscanrobot and try again**")
 ...
 CMD_HELP.update(
     {
         ...
         \n\n • **Syntax :** `{cmd}ustat` or `.ustats`\
         \n • **Function : **To check which group the person belongs to\
     "
     }
 )
