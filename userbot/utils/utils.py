...
async def autobot():
    ...
    await bot.send_message(
        BOTLOG_CHATID, "**IS MAKING A TELEGRAM BOAT FOR YOU AT @BotFather**"
    )
    ...
    if who.username:
        ...
        username = "alphonse" + (str(who.id))[5:] + "ubot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "alphonse" + (str(who.id))[6:] + str(ran) + "ubot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await asyncio.sleep(1)
            await bot.send_message(
                bf, f"✨ Owner ~ {who.first_name} ✨\n\n✨ Powered By ~ @TeamAlphonse ✨"
            )
            await bot.send_message(
                BOTLOG_CHATID,
                f"**SUCCESSFULLY CREATING A TELEGRAM BOT WITH USERNAME@{username}**",
            )
            await bot.send_message(
                BOTLOG_CHATID,
        ...
        LOGS.info(
            "Please Delete Some of your Telegram Bots on @Botfather or Set Var BOT_TOKEN with bot token"
        )
        sys.exit(1)
