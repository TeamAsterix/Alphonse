    ...
    try:
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    manevent,
                    "**Please reply message or provide user ID/username!**",
                    60,
                )
            return None, None
    except Exception as e:
...
async def checking():
    gocheck = str(pybase64.b64decode("QEFscGhvbnNlVXBkYXRlcw=="))[2:13]
    checker = str(pybase64.b64decode("QEFscGhvbnNlU3VwcG9ydA=="))[2:17]
    try:
